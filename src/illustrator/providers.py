...
class FluxSchnellVertexProvider(ImageGenerationProvider):
    """Google Vertex AI Flux Schnell model provider."""
    ...
    def __init__(
        ...
        *,
        ...
        huggingface_api_key: str | None = None,
        huggingface_config: HuggingFaceConfig | None = None,
        flux_schnell_vertex_endpoint_url: str | None = None,
        gcp_credentials: str | None = None,
    ) -> None:
        """Initialize Flux Schnell Vertex provider."""
        super().__init__(
            prompt_engineer=prompt_engineer,
            llm_provider=llm_provider,
            llm_model=llm_model,
        ...
        self.gcp_project_id = gcp_project_id
        self.endpoint_url = flux_schnell_vertex_endpoint_url
        self.gcp_credentials = gcp_credentials
    ...
    def get_provider_type(self) -> ImageProvider:
        """Return Flux Schnell Vertex provider type."""
        return ImageProvider.FLUX_SCHNELL_VERTEX

    @resilient_async(
        max_attempts=3,
        fallback_functions={
            'simplified_processing': _async_generation_failure('flux_schnell_vertex')
        }
    )
    async def generate_image(
        self,
        ...
        **kwargs
        ...
        """Generate image using Flux Schnell via Google Vertex AI."""
        if not self.endpoint_url:
            return {
                'success': False,
                'error': 'No Vertex AI endpoint URL configured for Flux Schnell'
            }
        ...
        try:
            ...
            logger.error(f"Flux Schnell Vertex generation error: {e}")
            return {
                'success': False,
                'error': f"Generation failed: {str(e)}"
            }
...
class ProviderFactory:
    ...
    def create_provider(
        ...
        **credentials
        ...
        elif provider_type == ImageProvider.FLUX_SCHNELL_VERTEX:
            project_id = credentials.get('google_project_id') or credentials.get('gcp_project_id')
            if not project_id:
                raise _credential_error(
                    provider_type,
                    "Google Cloud project ID required for Flux Schnell Vertex provider"
                )
            return FluxSchnellVertexProvider(
                project_id,
                flux_schnell_vertex_endpoint_url=credentials.get('flux_schnell_vertex_endpoint_url'),
                gcp_credentials=credentials.get('google_credentials'),
                **common_llm_kwargs,
            )
